...
std::unique_ptr<schema::Schema> getTPCHSchemaLight() {
    ...
    std::unique_ptr<schema::Schema> getTPCHSchemaLight2() {
        std::vector<schema::Table> tables {
                schema::Table(
                        "customer",
                        {
...
TEST(SegmentTest, SchemaSerialiseEmptySchema) {
    ...
    TEST(SegmentTest, SchemaSerialiseTPCHLight2) {
        BufferManager buffer_manager(1024, 10);
        {
            SchemaSegment schema_segment_1(103, buffer_manager);
            auto schema_1 = getTPCHSchemaLight2();
            schema_segment_1.set_sp_segment(5);
            schema_segment_1.set_fsi_segment(6);
            schema_segment_1.increment_sp_count();
